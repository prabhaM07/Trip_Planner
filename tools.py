...
def web_search(query: str) -> str:
    ...
    try:
        ...
        # # If Tavily gives a direct AI answer
        # if results.get("answer"):
        #     answer_text = f"Answer: {results['answer']}\n"
        #     if results.get("results"):
        #         answer_text += "Sources:\n"
        #         for i, r in enumerate(results["results"][:3], 1):
        #             answer_text += f"{i}. {r.get('title', 'No title')} - {r.get('url', '')}\n"
        #     return answer_text

        # Otherwise, just list top search results
